...
const DiscountComponent: React.FC<DiscountComponentProps> = ({
  ...
  const fetchCoupons = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${api_getCoupons}?active=true`);
  ...
  return (
    <div className="w-full sm:w-[450px] max-w-[100vw] min-w-[300px] bg-white h-screen p-4 sm:p-6 border-l border-gray-200 flex flex-col mt-10 overflow-hidden">
      ...
      {/* Manual Code Input */}
      <div className="flex items-center mb-6 sm:mb-8 rounded-full overflow-hidden border border-orange-300 bg-white">
        <input
          value={manualCode}
      ...
      {/* Title */}
      <h2 className="text-center font-bold text-xl sm:text-2xl text-black mb-4 sm:mb-6 tracking-wide [font-family:'Barlow_Condensed',Helvetica]">
        AVAILABLE COUPONS
      </h2>
